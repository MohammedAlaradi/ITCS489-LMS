...
try {
    ...
    // Keep $books empty on error; optionally you can log $e->getMessage()
}
?>
<!DOCTYPE html>
<html lang="en">
...
<main class="container mt-4">
    ...
    <section>
        ...
        <div class="row">
        <?php
        // Render server-side book cards
        if (!empty($books)) {
            foreach ($books as $i => $book) {
                $title = htmlspecialchars($book['Title'] ?? 'Untitled');
                $genre = htmlspecialchars($book['Genre'] ?? 'Unknown');
                $year = htmlspecialchars($book['YearofPublish']  ?? 'N/A');
                $author = htmlspecialchars($book['Author'] ?? 'Unknown');
                $copies = intval($book['Copies'] ?? 0);
                $image = '../ULiblogo.png';
                $buttonClass = $copies === 0 ? 'reserve-btn' : 'borrow-btn';
                echo "<div class=\"col-md-6 mb-2\">\n";
                echo "  <div class=\"card mb-2\" style=\"max-width: 680px;\">\n";
                echo "    <div class=\"row g-0\">\n";
                echo "      <div class=\"col-md-4\">\n";
                echo "        <img src=\"$image\" class=\"img-fluid rounded-start\" alt=\"Book Cover\">\n";
                echo "      </div>\n";
                echo "      <div class=\"col-md-8\">\n";
                echo "        <div class=\"card-body\">\n";
                echo "          <h5 class=\"card-title\">$title</h5>\n";
                echo "          <p class=\"card-text\"><strong>Genre:</strong> $genre</p>\n";
                echo "          <p class=\"card-text\"><strong>Year Of Publish:</strong> $year</p>\n";
                echo "          <p class=\"card-text\"><strong>Author:</strong> $author</p>\n";
                if ($copies === 0) {
                    echo "          <button class=\"btn btn-primary w-100 rounded reserve-btn\">Reserve</button>\n";
                } else {
                    echo "          <button class=\"btn btn-primary w-100 rounded borrow-btn\">Borrow</button>\n";
                }
                echo "        </div>\n";
                echo "      </div>\n";
                echo "    </div>\n";
